...
# Bio-Ethanol -------------------------------------------------------------
# Only data from EIA because I have not found the one from IEA
# (http://dx.doi.org/10.1787/data-00550-en seems to require payment)

# eth <- merge(eth_eia, eth_iea, all = TRUE)
eth_eia[, `:=`(value = value_eia, value_eia = NULL)]
eth <- eth_eia
